...
namespace opensesame {
		...
		if (!Environment::AccountFromDB(address, acc)) {
			...
			if (!storagekey.empty()) {
				...
				if (acc->GetMetaData(storagekey, value_ptr)) {
					metadatas[(Json::UInt)0] = opensesame::Proto2Json(value_ptr);
				}
			}
			else {
	...
	void WebServer::GetGenesisAccount(const http::server::request &request, std::string &reply) {
		std::string address;
		Storage::Instance().account_db()->Get(opensesame::General::KEY_GENE_ACCOUNT, address);
		...
		if (!Environment::AccountFromDB(address, acc)) {
			...
			if (!metadata_key.empty()) {
				...
				if (acc->GetMetaData(metadata_key, value_ptr)) {
					result[metadata_key] = opensesame::Proto2Json(value_ptr);
				}
			}
			else {
				std::vector<protocol::KeyPair> metadata;
	...
	void WebServer::Debug(const http::server::request &request, std::string &reply) {
		...
		Json::Value ret;
		ret["ret"] = opensesame::Proto2Json(x);
	...
	void WebServer::GetTransactionHistory(const http::server::request &request, std::string &reply) {
		...
		opensesame::KeyValueDb *db = opensesame::Storage::Instance().ledger_db();
	...
	void WebServer::GetModulesStatus(const http::server::request &request, std::string &reply) {
		utils::ReadLockGuard guard(opensesame::StatusModule::status_lock_);
		Json::Value reply_json = *opensesame::StatusModule::modules_status_;

		reply_json["keyvalue_db"] = Json::Value(Json::objectValue);
		opensesame::Storage::Instance().keyvalue_db()->GetOptions(reply_json["keyvalue_db"]);
		reply_json["ledger_db"] = Json::Value(Json::objectValue);
		opensesame::Storage::Instance().ledger_db()->GetOptions(reply_json["ledger_db"]);
		reply_json["account_db"] = Json::Value(Json::objectValue);
		opensesame::Storage::Instance().account_db()->GetOptions(reply_json["account_db"]);
