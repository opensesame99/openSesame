...
int main(int argc, char *argv[]){
	...
	opensesame::NotaryMgr::InitInstance();
	...
	do {
		...
		opensesame::NotaryMgr &cross_mgr = opensesame::NotaryMgr::Instance();
		if (!cross_mgr.Initialize()) {
			LOG_ERROR("Initialize cross manager failed");
			break;
		}
		object_exit.Push(std::bind(&opensesame::NotaryMgr::Exit, &cross_mgr));
	...
	opensesame::NotaryMgr::ExitInstance();
