...
namespace opensesame {
	...
	opensesameChainRpc::opensesameChainRpc(INotifyRpc *notify, const ChainConfigure &config) : BaseChainRpc(notify, config), private_key_(config.private_key_){
	}

	opensesameChainRpc::~opensesameChainRpc(){

	}

	bool opensesameChainRpc::GetCommContractInfo(const std::string &address, CommContractInfo &comm_info){
		...
		do
		{
			...
			if (context.empty()){
				LOG_ERROR("%s:opensesame chain get comm contract info %s error", config_.output_data_.c_str(), config_.comm_contract_.c_str());
				return false;
			}
			Json::Value result;
		...
		if (!send_relay_obj.isMember("notary_list")){
			LOG_ERROR("SesameChain no notary list");
			return false;
		}
		...
		do
		{
			...
			std::string context = HttpGet(PackUrl(path));
			if (context.empty()){
				LOG_ERROR("%s:opensesame chain get ledger error", config_.output_data_.c_str());
				return false;
			}
	...
	bool opensesameChainRpc::GetProposal(const CommContractInfo &comm_info, const std::string &address, ProposalType type, int64_t seq, ProposalInfo &proposal_info){
		...
		std::string context = HttpGet(PackUrl(path));
		if (context.empty()){
			LOG_TRACE("%s:opensesame chain get proposal info %s error", config_.output_data_.c_str(), config_.comm_contract_.c_str());
			return false;
		}
	...
	utils::StringVector opensesameChainRpc::CommitVotedProposals(const ProposalInfoVector &proposal_vector){
		...
		if (context.empty()){
			LOG_ERROR("%s:opensesame chain get getTransactionBlob error", config_.output_data_.c_str());
			return hash_array;
		}
	...
	void opensesameChainRpc::OnTimer(){
		//获取nonce值
		std::string path = utils::String::Format("/getAccount?address=%s", config_.notary_address_.c_str());
		std::string context = HttpGet(PackUrl(path));
		if (context.empty()){
			LOG_ERROR("%s:opensesame chain get getAccount error", config_.output_data_.c_str());
			return;
		}
	...
	std::string opensesameChainRpc::PackUrl(const std::string &path){
	...
	std::shared_ptr<BaseChainRpc> CreateChainRpc(INotifyRpc *notify, const ChainConfigure &config){
		...
		if (config.chain_name_ == "opensesame"){
			return std::make_shared<opensesameChainRpc>(notify, config);
		}
