...
namespace opensesame {
	...
	bool LedgerManager::Initialize() {
		...
		opensesame::General::SetSelfChainId(lclheader.chain_id());
	...
	bool LedgerManager::CreateGenesisAccount() {
		...
		batch->Put(opensesame::General::KEY_LEDGER_SEQ, "1");
		batch->Put(opensesame::General::KEY_GENE_ACCOUNT, Configure::Instance().genesis_configure_.account_);
	...
	void LedgerManager::CreateHardforkLedger() {
		...
		do {
			...
			opensesame::KeyValueDb *ledger_db = opensesame::Storage::Instance().ledger_db();
			...
			batch->Put(opensesame::General::KEY_LEDGER_SEQ, utils::String::ToString(header->seq()));
			batch->Put(opensesame::General::LAST_PROOF, "");

			ValidatorsSet(batch, new_validator_set);

			//Write ledger db
			WRITE_BATCH batch_ledger;
			batch_ledger.Put(opensesame::General::KEY_LEDGER_SEQ, utils::String::ToString(header->seq()));
	...
	bool LedgerManager::CloseLedger(const protocol::ConsensusValue& consensus_value, const std::string& proof) {
		...
		account_db_batch->Put(opensesame::General::KEY_LEDGER_SEQ, utils::String::Format(FMT_I64, ledger_seq));
		...
		//proof
		account_db_batch->Put(opensesame::General::LAST_PROOF, proof);
		account_db_batch->Put(opensesame::General::STATISTICS, statistics_.toFastString());
		proof_ = proof;
	...
	void LedgerManager::OnRequestLedgers(const protocol::GetLedgers &message, int64_t peer_id) {
		...
		if (ret) {
			opensesame::WsMessagePointer ws = std::make_shared<protocol::WsMessage>();
	...
	Result LedgerManager::DoTransaction(protocol::TransactionEnv& env, LedgerContext *ledger_context) {
		...
		TransactionFrm::pointer new_tx = std::make_shared<opensesame::TransactionFrm >(env);
