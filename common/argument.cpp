...
namespace opensesame {
	...
	bool Argument::Parse(int argc, char *argv[]) {
		if (argc > 1) {
            ...
            if (s == "--console-with-cmd") {
				...
				if (argc > 2) {
					printf("%s\n", (utils::Aes::CryptoHex(argv[2], opensesame::GetDataSecuretKey())).c_str());
				}
				else {
			...
			//	printf("%s\n", utils::Aes::HexDecrypto(argv[2], opensesame::GetDataSecuretKey()).c_str());
			//	return true;
			//}
			else if (s == "--sm3" ) {
				if (argc > 2) {
				...
				std::string sk_hash = utils::String::BinToHexString(opensesame::HashWrapper::Crypto(opensesame::GetDataSecuretKey())).substr(0, 2);
#ifdef SVNVERSION
				printf("%s;%u;secure:%s;git:" SVNVERSION "\n", General::opensesame_VERSION, General::LEDGER_VERSION, sk_hash.c_str());
#else
				printf("%s;%u;secure:%s\n", General::opensesame_VERSION, General::LEDGER_VERSION, sk_hash.c_str());
#endif 
				return true;
				...
				result["private_key_aes"] = utils::Aes::CryptoHex(private_key, opensesame::GetDataSecuretKey());
				...
				opensesame::PublicKey pub_key(argv[2]);
	...
	bool Argument::SignDataWithKList(int argc, char *argv[]) {
		...
		for (size_t i = 0; i < json_items.size(); i++) {
			...
			opensesame::PrivateKey priv_key(str_priv_key);
	...
	void Argument::ShowNodeId(int argc, char *argv[]) {
		...
		opensesame::PrivateKey private_key(utils::Aes::HexDecrypto(argv[2], opensesame::GetDataSecuretKey()));
