...
namespace opensesame {
	MonitorManager::MonitorManager() : Network(SslParameter()) {
		...
		request_methods_[monitor::MONITOR_MSGTYPE_opensesame] = std::bind(&MonitorManager::OnopensesameStatus, this, std::placeholders::_1, std::placeholders::_2);
	...
	opensesame::Connection * MonitorManager::CreateConnectObject(opensesame::server *server_h, opensesame::client *client_, opensesame::tls_server *tls_server_h, 
		opensesame::tls_client *tls_client_h, opensesame::connection_hdl con, const std::string &uri, int64_t id) {
	...
	bool MonitorManager::OnMonitorHello(protocol::WsMessage &message, int64_t conn_id) {
		...
		do {
			...
			reg.set_blockchain_version(opensesame::General::opensesame_VERSION);
			reg.set_data_version(opensesame::General::MONITOR_VERSION);
	...
	bool MonitorManager::OnopensesameStatus(protocol::WsMessage &message, int64_t conn_id) {
		monitor::opensesameStatus opensesame_status;
		// Get the status of opensesame
		GetopensesameStatus(opensesame_status);
		...
		// Send the response of opensesame status
		if (NULL == monitor || !monitor->SendResponse(message, opensesame_status.SerializeAsString(), ignore_ec)) {
			bret = false;
			LOG_ERROR("Failed to send opensesame status from ip(%s) (%d:%s)", monitor->GetPeerAddress().ToIpPort().c_str(),
				ignore_ec.value(), ignore_ec.message().c_str());
		}
		return bret;
	}
	...
	Connection * MonitorManager::GetClientConnection() {
		opensesame::Connection* monitor = NULL;
	...
	bool MonitorManager::GetopensesameStatus(monitor::opensesameStatus &opensesame_status) {
		...
		monitor::GlueManager *glue_manager = opensesame_status.mutable_glue_manager();
		...
		monitor::PeerManager *peer_manager = opensesame_status.mutable_peer_manager();
