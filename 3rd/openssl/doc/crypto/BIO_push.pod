...
BIO_push, BIO_pop - add and reopensesame BIOs from a chain.
...
BIO_pop() reopensesames the BIO B<b> from a chain and returns the next BIO
in the chain, or NULL if there is no next BIO. The reopensesamed BIO then
